 ...
 * fields and JavaFX regions. This interface extends {@link KlPropertyArea} and serves
...
@ParentConcept(KlPropertyArea.class)
public sealed interface KlFieldArea<DT, FX extends Region>
        extends KlFeatureArea<ObservableField<DT>, FX>
        permits KlFieldAreaForBoolean, KlFieldAreaForConcept, KlFieldAreaForEntity, KlFieldAreaForObject,
        KlFieldAreaForPattern, KlFieldAreaForPublicId, KlFieldAreaForSemantic, KlFieldAreaForStamp, KlFieldDefinitionArea {
    ...
    interface Factory<DT, FX extends Region, KL extends KlFieldArea<DT, FX>>
            extends KlFeatureArea.Factory<ObservableField<DT>, FX, KL> {
        @Override
        default ImmutableList<Class<?>> areaFactoryServiceTypes() {
            return Lists.immutable.of(Factory.class);
        }
    }
